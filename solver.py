...
def solve(message, a_id, q_id, ans, token):
    headers = {'Authorization': 'Bearer ' + token}
    quest_data = requests.get(f"https://api.matetech.ru/api/public/companies/3/test_attempts/{crypter.encrypt(a_id)}/question/{crypter.encrypt(q_id)}", headers=headers).json()
    ...
    if quest_data['data']['type'] in ['radio', 'checkbox', 'sort']:
        ...
        return "все ок" if requests.post(
            f"https://api.matetech.ru/api/public/companies/3/question_attempts/{q_a_id}/answer", json=res,
            headers=headers).status_code == 200 else "все плохо, пишите @danosito"
    else:
